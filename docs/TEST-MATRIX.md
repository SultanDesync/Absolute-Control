...
The current architecture-hardening tree has no new human visual/UX acceptance. Its automated
product process covers the release build, native contracts, SDK tests, generated fixture
check/compile, catalogue validation, complete SWF provenance, artifact fixtures, canonical
manifest, and compatibility ZIP. Current development tests additionally preserve the earlier
grid-column/flags-only ABI layouts, validate capability-gated row-to-Choice associations and their
fallback, exercise keyboard/controller activation of the inline Choice, and prove positional tier
normalization. Other runtime rows below are retained evidence from earlier artifacts unless stated.
...
| Provider page-open command | Build verified; in-game subscriber regression pending | ABI lifecycle/coalescing tests plus native task and Scaleform source-contract tests | The size/capability-gated ABI-v1 tail validates registered routes, coalesces to the newest request, schedules UI-thread work, and dispatches page selection from the bridge heartbeat. Missing/old hosts retain provider fallback UI; no input is synthesized. |
...
| Semantic composition C0-C2 | Build verified; runtime visual/UX pending | `composition_registry_test`, `semantic_composition_integration_test`, Scaleform compile/source architecture, release/research artifact boundary, HOTAS composition/subscriber/telemetry tests, and `tools/process/validate-current.cmd` | C0 validates bounded trees, references, capacities, associations, immutable state, fallback, and callback leases. C1 adds semantic status/conditions/anchors. C2 adds same-page live slots and validates series/marker IDs against registered live descriptors. The independent six-card fixture exercises embedded live publication and pointer/keyboard/controller focus policy. HOTAS mechanically covers all 43 Flight Axes controls in 83 nodes with six anchors and live plots; in-game visual and physical-input acceptance remains pending. |
| Direct range landmarks | Build verified; HOTAS visual/feel acceptance pending | Scaleform source/architecture tests plus HOTAS subscriber and telemetry tests | Typed range markers use the ordinary coalesced draft-write lane, remain under provider validation and Apply/Cancel, and cannot fight an active physical set-by-feel gesture. Semantic zone colours, a white physical-lever marker, selected handles, and bounded zone selection are host-owned. Widths remain ordinary sliders because they are not a one-to-one marker value. |
| AbsoluteZero radial tuning lab | Build verified; in-game visual/feel acceptance pending | Live registry compatibility tests, Scaleform compilation/source contract, AbsoluteZero subscriber/configuration tests, and release builds | The host exact-gates the additive radial component, preserves prior full descriptor/frame sizes, renders the three-circle flight model, automatically consumes relative mouse motion while the page is visible, and runs tuned idle detection and centering without leaving demo mode. A dedicated Start/Stop control opts out without consuming ordinary menu clicks. It evaluates the provider's current radius, delay, rate, and polling controls through the gameplay-equivalent discrete exponential response. Adjacent sliders stay on the ordinary draft/Apply/Cancel lane. |
| Head-pose calibration surface | Build verified; in-game visual/feel acceptance pending | Live registry compatibility tests, Scaleform compilation/source contract, AHT API layout test, AHT release build, and source-built SWF | The host exact-gates the additive HeadPose descriptor/frame tails while retaining old RadialResponse full sizes. The Axes page renders profile pitch, top-view yaw, and roll horizon instruments with labeled negative/positive arcs, live/raw telemetry, configured center, and mirrored scalar sliders. AHT publishes copied atomic state and retains all 18 ordinary controls plus the deprecated scalar-only fallback. |
| Accessible grid and inline priority controls | Build verified; current-artifact runtime pending | Live registry/session/bridge/source tests plus Absolute Power positional normalization tests | The committed column and flags-only descriptor layouts remain accepted; malformed or duplicate associations fail closed; an absent capability leaves Choices in the ordinary list; retained same-page Choices open from pointer or keyboard/controller grid focus; and SetSegmentTier guarantees the activated pip acquires the requested tier while preserving Green→Yellow→Red ordering. |
| Absolute Head Tracking | Existing scalar integration verified; HeadPose runtime pending | Earlier Starfield 1.16.244 scalar run plus current AHT/host/SWF builds | General, 18-control asymmetric Axes, Aim Restriction, and Bindings pages retain provider-owned Apply/Cancel. The new capability-gated HeadPose channel is automated/build verified but has not yet received in-game visual and input-mode acceptance. |
...
| Xbox-compatible controller only | A/B/D-pad plus segmented-grid focus implemented; runtime pending | Prove physical traversal, grid Up/Down row selection, Left/Right adjustment, A-opening the row-associated priority Choice, popup selection, dirty Apply/Discard/Stay, conflict/capture cancellation, held/repeat suppression, no leaked command, and no ghost/stuck axis. |
...
| Controller/HOTAS binding capture | Core journey verified; conflict UX and visible recording state automated; expanded runtime matrix pending | Head Tracking and Absolute Power prove physical recording, persistence/action use, and shared pilot context. A centered host-owned recording panel names the target, gives axis/button/POV guidance, reports navigation lock, and shows the cancel input. Host tests cover presentation source, provider Error and Duplicate collision paths, and Reassign/Cancel transaction ownership. Still exercise POV, cancel, clear, timeout, restart/read-back, physical collision resolution, and generation reseed as one formal matrix. |
